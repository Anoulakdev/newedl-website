                  ...
                  <div className="container">
                     <div className="row align-items-center">
                        ...
                        {data.slice(0, 1).map((item, i) => (
                            <p key={i} className='text-white pt-2'>{moment(item.noti.start_date).format("DD-MM-YYYY")} ຫາ {moment(item.noti.end_date).format("DD-MM-YYYY")} | {item.noti.start_time}-{item.noti.end_time} | {item.noti.title_head}</p>
                        ))}
                        </div>
                        <div className="col-xl-2 col-lg-2 text-end mt-2">
                            <Link href="/turnoff" type='button' className='btn btn-light'>ເບິ່ງ​ເພີ່ມ​ເຕີມ</Link>
                        </div>
                     </div>
                  </div>
               </div>
